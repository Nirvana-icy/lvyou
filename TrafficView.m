...
{
    [super viewDidLoad];
    self.navigationItem.title = @"交通";
    
    dataArray = [[NSMutableArray alloc] init];
    MyLog(@"~~~一进来打印sidstr==%@",sidStr);
    //retain————————————————————————————————
    tableName = [[NSString  stringWithFormat:@"traffic%@",sidStr]retain];
    ...
    //**************用segmentedControl做两个按钮**************
    UIImage * image01 = [UIImage imageNamed:@"image_announcement_on"];
    UIImage * image02 = [UIImage imageNamed:@"image_announcement_on"];
    UIImage * image12 = [UIImage imageNamed:@"image_announcement_off"];
    UIImage * image22 = [UIImage imageNamed:@"image_announcement_off"];

    imageArrayNormal = [[NSArray alloc]initWithObjects:image01,image02,nil];
    
    imageArrayChoose = [[NSArray alloc] initWithObjects:image12,image22,nil];
    mSegmentedControl = [[UISegmentedControl alloc] initWithItems:imageArrayNormal];
    [mSegmentedControl setTitle:@"当地交通" forSegmentAtIndex:0];
    [mSegmentedControl setTitle:@"到达与离开" forSegmentAtIndex:1];
    mSegmentedControl.segmentedControlStyle = UISegmentedControlStyleBezeled;
    mSegmentedControl.frame = CGRectMake(60, 7, 200, 30);
    [mSegmentedControl addTarget:self action:@selector(segmentDidChange:) forControlEvents:UIControlEventValueChanged];
    mSegmentedControl.selectedSegmentIndex = 0;
    [vw addSubview:mSegmentedControl];
    [self.view addSubview:vw];
    
    //下载
    mhttpdownload = [[HttpDownload alloc] init];
    mhttpdownload.delegate = self;
    [mhttpdownload downloadFromUrl:EIGHTBTN_TRAFFIC_URL];
    ...
    if (hd.mData)
    {
        ...
        if (dictA)
        {
            ...
            else
            {
                ...
                NSMutableArray * zongArray = [[NSMutableArray alloc] initWithArray:[dictBB objectForKey:@"remote"]];
                ...
                if (0 != [dataArray count])
                {
                    ...
                    TrafficRemoteItem * trafficItem = [[TrafficRemoteItem alloc] init];
                    [[Database sharedDatabase] buildTable:sqlString withTableType:trafficItem];
...
-(void)downloadFaild:(HttpDownload *)hd
{
    ...
    TrafficRemoteItem * trafficRemoteItem = [[TrafficRemoteItem alloc] init];
    //从指定表读指定数据
    
    NSArray * array = [[Database sharedDatabase] selectFrom:tableName readItem:0 count:20 withItemType:trafficRemoteItem];
    [trafficRemoteItem release];
...
- (void)dealloc
{
    imageArrayNormal = nil;
    imageArrayChoose = nil;
    [super dealloc];
}
